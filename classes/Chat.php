...
class Chat {
    ...
    function getChatInteraction($chat_id, $user_id, $chat_type){
        ...
        $chat_interactions = app('db')->getOne('chat_interactions', 'id');
    ...
    function getPrivateChats($user_1, $user_2, $chat_room, $chat_id=false, $direction='up', $load_chats_from=false){
        if ($chat_id) { // when chat search
            ...
            app('db')->orderBy('c.time','desc');
            $chats1 = app('db')->get('private_chats c', array(0,10), 'c.*, u.user_name, u.first_name, u.last_name, u.avatar, u.user_type, \'private\' as chat_type');
            $_SESSION['last_loaded_up'] = (end($chats1)['id']?end($chats1)['id']:0);
            ...
            app('db')->orderBy('c.time','asc');
            $chats2 = app('db')->get('private_chats c', array(0,10), 'c.*, u.user_name, u.first_name, u.last_name, u.avatar, u.user_type, \'private\' as chat_type');
            $_SESSION['last_loaded_down'] = (end($chats2)['id']);
            ...
            $chats = app('db')->get('private_chats c', array(0,20), 'c.*, u.user_name, u.first_name, u.last_name, u.avatar, u.user_type, \'private\' as chat_type');
            if ($chats) {
                if ($direction == 'up') {
                    $_SESSION['last_loaded_up'] = (end($chats)['id']?end($chats)['id']:0);
                }else if($direction == 'down'){
